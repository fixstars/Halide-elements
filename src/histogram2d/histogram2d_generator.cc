...
public:
    ...
    GeneratorParam<int32_t> hist_width{"hist_width", 256};
    ...
    Func build() {
        ...
        Expr idx0 = cast<int32_t>(src0(r.x, r.y) * cast<uint32_t>(hist_width) / (cast<uint32_t>(type_of<T>().max()) + 1));
        Expr idx1 = cast<int32_t>(src1(r.x, r.y) * cast<uint32_t>(hist_width) / (cast<uint32_t>(type_of<T>().max()) + 1));
