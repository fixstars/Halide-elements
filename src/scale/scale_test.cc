...
//
// template<typename T>
// T* ref_bicubic(T* dst, const T* src, const int32_t src_width, const int32_t src_height,
    ...
    try {
        ...
        for (int y=0; y<out_height; ++y) {
            for (int x=0; x<out_width; ++x) {
                T actual = output(x, y);
                if (x == 0 && y == 0){//to check the actual values in two outputs at (0, 0)
                   printf("expect(%d, %d) = %d, actual(%d, %d) = %d\n",
